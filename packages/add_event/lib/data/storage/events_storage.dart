...
class EventsStorage {
  ...
  void init() {
    ...
    if (events != null) {
      ...
      _eventsListSubject.add(jsonList
          .map(
            (e) => EventModelMapperToEntity(EventModelDto.fromJson(e)).transform(),
          )
          .toList());
    } else {
  ...
  Future<void> saveEvents(List<EventModel> events) async {
    await shared.setString(
      _eventsStorageKey,
      jsonEncode(events.map((e) => EventModelMapperToDto(e).transform().toJson()).toList()),
    );
