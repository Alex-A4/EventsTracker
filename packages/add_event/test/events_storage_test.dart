...
void main() {
  ...
  final event1Json = EventModelMapperToDto(event1).transform().toJson();
  final event2Json = EventModelMapperToDto(event2).transform().toJson();
  ...
  group('EventsStorage.addEvent', () {
    ...
    test('addEvent when was data before', () async {
      SharedPreferences.setMockInitialValues({
        _storageKey: jsonEncode([event1Json]),
      });
      await initStorage();
  ...
  group('EventsStorage.removeEvent', () {
    ...
    test('removeEvent when there is data after removing', () async {
      SharedPreferences.setMockInitialValues({
        _storageKey: jsonEncode([event1Json, event2Json]),
      });
      await initStorage();
