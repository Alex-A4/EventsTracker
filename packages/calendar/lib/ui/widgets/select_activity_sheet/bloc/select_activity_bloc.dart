...
class SelectActivityCubit extends Cubit<SelectActivityState> {
  SelectActivityCubit(this.service, this.date) : super(const SelectActivityState.loading()) {
    sub = service.mappedEventsStream.listen(
      (events) => emit(
  ...
  final DateTime date;
  final CalendarActivitiesService service;

  late StreamSubscription<dynamic> sub;

  void selectEvent(EventModelWithStatistic event) {
    final s = state;
    if (s is _Data) {
  ...
  void selectTask(EventTaskWithStatistic task) {
    final s = state;
