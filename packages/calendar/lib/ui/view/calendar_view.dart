...
abstract class CalendarViewTexts {
      ...
      child: Column(
        children: [
          TableCalendar<CalendarDayTaskStatistics>(
            locale: texts.languageCode(context),
            onDaySelected: (selected, focused) {
              final date = cropDate(selected);
              final data = activities[date];
              if (data != null) {
                navigationAdapter.showCalendarEventsSheet(
                  context: context,
                  data: data,
                );
              } else if (date != null) {
                navigationAdapter.showCalendarSelectActivitySheet(context: context, date: date);
              }
            },
