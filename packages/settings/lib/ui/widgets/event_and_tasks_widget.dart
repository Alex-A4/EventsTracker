...
abstract class EventAndTasksSettingsWidgetTexts {
  /// Text for tile that shows user how much event completed in percent
  String eventCompletedCountAndPercent(String completedPercent);

  /// Texts for tile that shows user how much task completed in comparision with plan and in percent
  String taskCompletedCountAndPercent(String completed, String plan, String completedPercent);
}

/// Widget that allows display event and its tasks in a dropdown list
class EventAndTasksSettingsWidget extends StatefulWidget {
  ...
  final EventModelWithStatisticForSettings event;
  final SettingsNavigationAdapter navigationAdapter;
  final EventAndTasksSettingsWidgetTexts texts;
...
class _EventAndTasksSettingsWidgetState extends State<EventAndTasksSettingsWidget> {
      ...
      endActionPane: ActionPane(
        ...
        children: [
          SlidableAction(
            ...
            onPressed: (context) => widget.navigationAdapter.openEditEvent(
              context,
              widget.event.id,
            ),
          ),
  ...
  Widget _eventContent() {
    return ListTile(
      ...
      subtitle: Text(
        widget.texts.eventCompletedCountAndPercent(widget.event.completedGeneralPercent.toString()),
      ),
  ...
  Widget _taskItem(EventTaskWithStatisticForSettings task) {
    return ListTile(
      title: Text(task.taskName),
      subtitle: Text(
        widget.texts.taskCompletedCountAndPercent(
